...
function onViewDetail(data) {
  ...
  .then((response) => {
      ...
      if(response.exam.type != 'Tự do' && currentTime <  startTest) alert('Chưa đến giờ làm bài thi');
      else if(response.exam.type != 'Tự do'&& currentTime > endTest) alert('Quá giờ làm bài thi');
      else {
