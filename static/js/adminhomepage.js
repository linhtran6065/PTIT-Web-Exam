...
function onEdit(td) {
    if (createForm.classList.contains("inactive")) {
        ...
        document.getElementById("name").value = selectedRow.cells[0].querySelector("p").innerHTML;
        document.getElementById("description").value = selectedRow.cells[1].innerHTML;
        if (selectedRow.cells[2].querySelector("span").innerHTML == "Truy cập tự do") {
...
function onDelete(td) {
    if (confirm("Bạn có chắc chắn muốn xóa bài thi này?")) {
        ...
        name = examList.find((exam) => exam.name === row.cells[0].querySelector("p").innerHTML);
